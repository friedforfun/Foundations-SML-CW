...
fun CoTerm v (CID v) = if () then CI else false |
	CoTerm v (COM p) = if (not(Cfree v p)) then CAPP(CK, p) else false |
	CoTerm v (CAPP(p1, p2)) = if ((not(Cfree v p1)) andalso v = p2) then p1 else CAPP(CAPP(CS, CoTerm(v, p1)), CoTerm(v, p2));
